...
final class CharacterDetailViewModelTests: XCTestCase {
    ...
    func testInitStateValues() {
        let items: [CharacterDetailsViewItem] = [.init(type: .comics), .init(type: .series), .init(type: .stories)]
        XCTAssertEqual(viewModel.comicsItem.state, .loading)
        XCTAssertEqual(viewModel.seriesItem.state, .loading)
        XCTAssertEqual(viewModel.storiesItem.state, .loading)
        XCTAssertEqual(viewModel.eventsItem.state, .loading)
